...
impl<T> Term<T> {
    ...
    pub fn selection_to_string(&self) -> Option<String> {
        ...
        if is_block {
            ...
            self.write_line_to_string(&mut res, end.line, start.column..end.column, false, true);
        } else {
            self.write_bounds_to_string(&mut res, start, end, false);
        }
    ...
    pub fn write_bounds_to_string(
        ...
        for line in (start.line.0..=end.line.0).map(Line::from) {
            ...
            self.write_line_to_string(buf, line, start_col..end_col, esc_seqs, line == end.line);
        }
    }
